...
export const renderOnyx = (resume: any): string => {
    let html = loadTemplate("onyx", "resumes");
...
<div class="contents-sec">
    ...
    ${metaLine ? `<h5>${escapeHtml(metaLine)}</h5>` : ""}
    ${description}
</div>`;
        })
        .join("");
