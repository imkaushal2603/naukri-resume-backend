...
export function checkStructure(resume: ATSResume): ATSCheckResult {
  ...
  // Name - 15
  if (hasText(resume.fullName)) {
    score += 15;
  } else {
    addIssue(issues, "error", "Name is missing.", "fullName");
  }
  ...
  if (hasText(resume.summary)) {
    ...
    addIssue(issues, "suggestion", "Consider adding a professional summary.", "summary");
  }

  // Experience - 25
  if ((resume.resume_experience || []).length > 0) {
    score += 25;
  } else {
  ...
  // Education - 20
  if ((resume.resume_education || []).length > 0) {
    score += 20;
  } else {
    addIssue(issues, "warning", "Education section is missing.", "education");
  }

  // Skills - 20
  if ((resume.resume_skills || []).length > 0) {
    score += 20;
  } else {
    addIssue(issues, "warning", "Skills section is missing.", "skills");
  }
